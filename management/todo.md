...
- Payment method on the general tab is not used on the spreadsheet to determine whether to charge ACH fee or reverse wire fee — the pricing calculator does have that.
- Right now there is a box to put a promotion if there is one on the deal, just like the spreadsheet. It does not use it at all in the calculations, but if you knew all your types of promotions and set it up, the calculator could calculate the value with the promotion.
