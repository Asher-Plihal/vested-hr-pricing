...
function refreshSutaRowCells(tr, state) {
...
function addSutaRow(data) {
  ...
  tr.innerHTML = `
    ...
    <td><input type="text" name="suta_wses_turnover_${idx}" readonly style="background:#f7fafc;color:#718096;" value="—" /></td>
    <td><input type="text" name="suta_taxable_gws_${idx}" readonly style="background:#f7fafc;color:#718096;" value="—" /></td>
  `;
...
function updateSutaRows() {
  ...
  tbody.querySelectorAll('tr').forEach(tr => {
    ...
    if (!stateEl) return;
    refreshSutaRowCells(tr, stateEl.value);
  });
}
